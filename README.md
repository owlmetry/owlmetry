...
- **Notifications** — multi-channel notification system (in-app inbox, email, iOS push) with per-user preferences and pluggable channel adapters; powers issue digests, feedback alerts, and job completion alerts. Per-project frequency (`none | hourly | 6_hourly | daily | weekly`) controls cadence; users opt channels in or out per type. iOS push uses token-based APNs auth (optional `APNS_KEY_ID`, `APNS_TEAM_ID`, `APNS_KEY_P8`, `APNS_BUNDLE_ID`, `APNS_ENV` — without them, push is skipped and email + in-app keep working)
