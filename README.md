...
- **Store ratings & reviews** — App Store ratings (avg, count, current-version split) and individual reviews captured per app. Aggregate ratings refresh hourly via iTunes Lookup (no auth, no setup). Individual reviews come via the App Store Connect API integration — set up once per project with a Customer Support API key, sync on demand. Schema supports Play Store from day 1 (ingest deferred). Dashboard list with filters + by-country panel, rating badge on app cards, full CLI / MCP surface. See [docs/concepts/reviews](https://owlmetry.com/docs/concepts/reviews)
