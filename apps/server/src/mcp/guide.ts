...
- \`issue.new\` fires from \`issue_scan\` at the end of every hourly run, with one push per team summarizing all production issues that were just created or regressed. Defaults: in_app + ios_push on, email off. Bypasses any cadence throttle, so push lands within ~5 min of detection.
