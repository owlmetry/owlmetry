...
1. Ask the user for their **RevenueCat V2 Secret API key**. They generate it in RevenueCat dashboard → Project Settings → API Keys → + New secret API key. Required permissions: **Customer information → Customers Configuration → Read only** AND **Project configuration → Projects Configuration → Read only**. All other sections → No access.
