...
If you use [event attachments](/docs/concepts/attachments), add the `internal` location from `deploy/attachments-nginx-snippet.conf` inside the API server block. Downloads are served via `X-Accel-Redirect`, so nginx must map an internal URI prefix (`OWLMETRY_ATTACHMENTS_INTERNAL_URI`) to the on-disk directory (`OWLMETRY_ATTACHMENTS_PATH`, default `/opt/owlmetry-attachments`).
