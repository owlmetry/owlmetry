...
The endpoint accepts gzip-compressed request bodies. Set the `Content-Encoding: gzip` header. Compressed payloads must be under 1 MB (enforced via `Content-Length` check). The Swift SDK compresses payloads automatically.
...
- **Funnel events:** Messages matching `track:<step_name>` are parsed and inserted into the `funnel_events` table.
