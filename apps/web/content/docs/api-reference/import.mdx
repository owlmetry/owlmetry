...
| Dedup window | 48 hours | All time |
...
The import endpoint deduplicates events by `client_event_id` across **all time** (unlike ingest which uses a 48-hour window). This means:

- If you re-run an import script, previously imported events are silently skipped.
- Always include a `client_event_id` in each event to enable safe re-runs.
- Events without a `client_event_id` are always inserted (no dedup possible).
