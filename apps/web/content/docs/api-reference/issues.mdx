...
The `attachments` array contains up to 20 non-deleted attachments associated with the issue (most recent first). For pagination across all attachments linked to an issue, use the `/v1/attachments?issue_id=:issueId` endpoint.
