...
Funnels define a series of steps users should complete. Each step has a name and an event filter (matching on `message` and/or `screen_name`). SDKs track funnel steps via `track(stepName)` which emits events with the message `"track:<stepName>"`.
  ...
  "funnels": [
    {
      ...
      "steps": [
        {
          "name": "Sign Up",
          "event_filter": { "message": "track:sign-up" }
        },
        {
          "name": "Profile Setup",
          "event_filter": { "message": "track:profile-setup" }
        },
        {
          "name": "First Action",
          "event_filter": { "message": "track:first-action" }
        }
      ],
...
| `event_filter` | object | Yes | Must have at least `message` or `screen_name`. |
| `event_filter.message` | string | No | Match events with this exact message. |
...
// Request (all fields optional)
{
  ...
  "steps": [
    {
      "name": "New Step 1",
      "event_filter": { "message": "track:new-step" }
    }
  ]
}
...
|-----------|------|-------------|
| `since` | string | ISO 8601 timestamp (default: 30 days ago). |
