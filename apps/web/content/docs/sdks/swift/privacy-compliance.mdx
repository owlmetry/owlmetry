...
- We use an SDK-owned anonymous identifier (`owl_anon_*`) stored only in the host app's UserDefaults — never shared across apps.
...
|---|---|---|
| `NSPrivacyAccessedAPICategoryUserDefaults` | `CA92.1` | Persist the SDK's anonymous user ID and Apple Search Ads capture state in your app's own UserDefaults suite |
