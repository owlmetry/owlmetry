...
All events emitted by the SDK automatically include an `experiments` field containing the current set of assignments. You do not need to pass experiment data manually -- it is attached to every `info()`, `debug()`, `warn()`, `error()`, `step()`, `startOperation()`, `recordMetric()`, and all automatic lifecycle events.
...
// All step() calls automatically carry the experiment assignment
