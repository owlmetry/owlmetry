...
```swift
Owl.debug("Cache hit for user preferences", screenName: "Home", customAttributes: ["key": "user_prefs"])
```
...
```swift
Owl.warn("Slow network response", customAttributes: ["latency_ms": "1200", "endpoint": "/api/feed"])
```
...
```swift
Owl.error("Failed to load profile image", screenName: "Profile", customAttributes: ["error": error.localizedDescription])
```
...
- **Use `customAttributes` for structured data.** Anything you might want to filter, group, or search on later should be an attribute rather than embedded in the message string.
