...
The SDK's live path only populates numeric IDs — Apple's AdServices Attribution API doesn't return human-readable names. If a project has the [RevenueCat integration](/docs/concepts/integrations) enabled **and** RC's Advanced Apple AdServices integration is configured with ASA Campaign Management API credentials, RC resolves those IDs to names server-side and exposes them as subscriber attributes. OwlMetry's RC sync and webhook paths then fill `asa_campaign_name`, `asa_ad_group_name`, and the literal `asa_keyword` search term on top of the numeric IDs. Those fields are additive — the live SDK flow and the RC backfill never overwrite each other.
