...
Use `Owl.track()` to emit named step completions that feed into [funnel analytics](/docs/concepts/funnels). Each call sends an info-level event with a `track:<stepName>` message that the server matches against funnel step definitions.
...
Funnel steps recorded by the SDK are matched against funnel definitions on the server. Define your funnels in the dashboard or CLI with the step names you use in `track()` calls. Each step's `event_filter` matches on the `message` field (which will be `track:<stepName>`).
