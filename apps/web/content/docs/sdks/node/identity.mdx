...
The returned `ScopedOwl` has the same API as `Owl` -- `info()`, `debug()`, `warn()`, `error()`, `step()`, `startOperation()`, and `recordMetric()`.
...
`sessionId` must be a UUID string; passing a non-UUID throws synchronously. The Swift SDK's `Owl.sessionId` is already a UUID, so forwarding it verbatim from the client Just Works.
