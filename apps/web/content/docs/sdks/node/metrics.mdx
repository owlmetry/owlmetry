...
`Owl.startOperation(metric, attrs?)` returns an `Operation` object and emits a `start` phase event immediately.
...
Call exactly one of these methods on the returned `Operation`:
