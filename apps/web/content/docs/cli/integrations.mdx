...
Copies a configured integration from one project to another within the same team — handy for re-using the same RevenueCat or Apple Search Ads credentials across a multi-project team without retyping them. Credentials are **duplicated, not shared**: rotating the source later does not update the copy. For RevenueCat, the API key is copied but a **fresh webhook secret is generated** on the target (each project has its own webhook URL). For Apple Search Ads, a **fresh keypair is generated** on the target and the client/team/key/org IDs are cleared — the target goes into "pending setup" mode, and you upload the new public key to Apple (under the destination project's API user) before it enables. Requires admin role and `integrations:write` on the target. Fails with 409 if the target already has a non-deleted integration for that provider, 404 if the source has none, and 403 for cross-team copies.
