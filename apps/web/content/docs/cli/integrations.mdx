...
The `--api-key` is a RevenueCat **V2 Secret API key** (generated in RevenueCat → Project Settings → API Keys → + New secret API key). Required permissions: **Project configuration → Projects → Read only** AND **Customer information → Customers Configuration → Read only**; all other sections → No access.
