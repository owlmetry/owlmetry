...
|------|----------|-------------|
| `--project <id>` | No | Filter by project UUID |
...
| `--session <id>` | No | Filter by session UUID |
| `--screen <name>` | No | Filter by screen name |
...
owlmetry events --app 550e8400-... --level error --since 7d

# Events for a specific user session
owlmetry events --session a1b2c3d4-...
