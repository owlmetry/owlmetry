...
owlmetry metrics list --team-id <id>
```

Lists metric definitions for a single project, or every metric across every accessible project in a team. `--project-id` and `--team-id` are mutually exclusive; one is required. The team-scoped variant prepends a `Project` column to the table output so you can tell which project owns each metric.
...
Shows the metric definition including name, slug, description, aggregation rules, and documentation.
