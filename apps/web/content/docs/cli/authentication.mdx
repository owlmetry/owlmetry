...
Both `--endpoint` and `--api-key` are required globals for this command. Optionally pass `--ingest-endpoint <url>` to override the ingest endpoint (defaults to the API endpoint for self-hosted instances, or `https://ingest.owlmetry.com` when targeting `https://api.owlmetry.com`).

The command validates the key by calling the `/v1/auth/whoami` endpoint, fetches the team info, and saves the config (merging with any existing team profiles).
