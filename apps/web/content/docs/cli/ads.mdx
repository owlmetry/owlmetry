...
Default `table` format renders ranked tables with columns: name (with `[paused]` / `[deleted]` status badge inline when applicable), users, paying users, revenue, ARPU. **Spend** and **ROAS** columns appear automatically once at least one row carries a non-null `total_spend_usd` — ROAS is colored green / amber / red against the 1.0x and 0.5x thresholds. The summary line above the table adds total spend + overall ROAS alongside total revenue, with a separate `Spend synced` timestamp; a yellow `currency_warning` line surfaces when the project's Apple Search Ads org reports in a non-USD currency (spend stays blank and ROAS unavailable until USD support lands).
