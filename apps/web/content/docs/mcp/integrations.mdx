...
Copy a configured integration from one project to another within the same team. Credentials are **duplicated, not shared** — rotating the source later does not update the copy. For RevenueCat, the API key is copied but a fresh webhook secret is generated on the target. For Apple Search Ads, a fresh keypair is generated on the target and the client/team/key/org IDs are cleared — the target goes into "pending setup" mode, and the user has to upload the new public key to Apple (under the destination project's API user) before it enables. Requires `integrations:write` permission **and** admin role.
