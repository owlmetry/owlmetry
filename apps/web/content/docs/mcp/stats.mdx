...
Aggregation backfills run via [`trigger-job`](/docs/mcp/jobs#trigger-job) — pass `job_type: "stats_aggregate_daily"` or `"stats_aggregate_hourly"` with `start` / `end` (and optional `project_id`) params. The job DELETEs the target bucket range and re-inserts in a single transaction, so re-running is safe. Use [`get-job`](/docs/mcp/jobs#get-job) on the returned `job_run_id` to monitor progress. Aggregation tables are explicitly excluded from retention pruning, so historical buckets stay queryable indefinitely.
