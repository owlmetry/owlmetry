...
8. **Resolve or escalate** — `resolve-issue` with the fix version once patched (the version is required so regression detection has something to compare against). Use `silence-issue` if there's nothing to fix or no version exists. Leave the comment for the team to act on otherwise.
...
Mark an issue as resolved. The fix version is **required** — it powers regression detection (any later version reporting the same error auto-flips the issue to `regressed`). If you don't have a fix version, use [`silence-issue`](#silence-issue) instead.
