...
Get details of a specific issue, including occurrences (sessions affected), comments, fingerprints, and linked attachments. Each occurrence includes a `session_id` — pass it to [`query-events`](/docs/mcp/events#query-events) or [`investigate-event`](/docs/mcp/events#investigate-event) to reconstruct the full session timeline around the error.
