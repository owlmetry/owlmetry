...
4. **Reconstruct the session** — Use [`query-events`](/docs/mcp/events#query-events) with the occurrence's `session_id` to see every event in that session — what screens the user visited, what actions they took, and what led up to the error.
