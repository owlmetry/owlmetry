...
Backfills are idempotent. Each per-kind aggregator runs `DELETE` + two `INSERT … SELECT … GROUP BY` passes (per-app + project-rollup) inside a single transaction per bucket range. Re-running clears the slate first so a row that disappeared at source (e.g. an event later deleted) doesn't survive in the rollup.
