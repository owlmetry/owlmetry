...
3. **Configure the webhook** in RevenueCat dashboard (Settings → Webhooks → + New Webhook) using the values from `webhook_setup`:
   ...
   Lost the post-create output? Re-reveal the same webhook setup any time:
...
- Powers `app_store_connect_reviews_sync` — a project-scoped job that calls `GET /v1/apps/{id}/customerReviews` for every Apple app in the project that has an `apple_app_store_id`.
...
```
owlmetry integrations add app-store-connect \
  --project-id <id> \
  --issuer-id <uuid> \
  --key-id <ABC1234567> \
  --private-key-p8-file ./AuthKey_ABC1234567.p8
owlmetry integrations test app-store-connect --project-id <id>
owlmetry integrations sync app-store-connect --project-id <id>
```
