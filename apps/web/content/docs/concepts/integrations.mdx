...
1. **Generate a RevenueCat V2 Secret API key** in the RevenueCat dashboard (Project Settings → API Keys → + New secret API key). Set these permissions:
   - **Project configuration → Projects** → **Read only**
...
1. Use the OwlMetry CLI to add the RevenueCat integration to my project with my
   RC V2 secret API key (needs Project configuration → Projects → Read only AND
   Customer information → Customers Configuration → Read only, everything else
   No access). Use `/owlmetry-cli` for CLI syntax.
