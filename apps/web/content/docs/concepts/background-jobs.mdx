...
System jobs use [cron expressions](https://crontab.guru/) for scheduling. The server checks every 60 seconds which scheduled jobs are due. If a job is still running when its next scheduled time arrives, that run is skipped.
