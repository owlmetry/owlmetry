...
| `issue_scan` | Issue Scan | System | Hourly | Clusters new error events into issues via fingerprinting |
| `issue_notify` | Issue Notify | System | Hourly @ :05 | Sends per-project issue digest emails; silent when nothing new or regressed |
| `app_version_sync` | App Version Sync | System | Hourly @ :15 | Refreshes each app's `latest_app_version` (iTunes Lookup for Apple, computed from production events otherwise). System-scoped — cannot be triggered via the API, CLI, or MCP. See [Latest Version Detection](/docs/concepts/latest-version-detection) |
