...
| `app_version_sync` | App Version Sync | System | Hourly @ :15 | Refreshes each app's `latest_app_version`, aggregate ratings (avg + count, current + all-time), and `apple_app_store_id` (iTunes Lookup for Apple, computed from production events otherwise). System-scoped — cannot be triggered via the API, CLI, or MCP. See [Latest Version Detection](/docs/concepts/latest-version-detection) |
| `app_store_connect_reviews_sync` | App Store Connect Reviews Sync | Project | Manual | Pulls App Store reviews via the App Store Connect customerReviews API for every Apple app in the project that has an `apple_app_store_id`. Requires the project to have an active App Store Connect [integration](/docs/concepts/integrations#app-store-connect). Idempotent — re-runs stop at the first review already in the DB. See [Store Reviews](/docs/concepts/reviews) |
