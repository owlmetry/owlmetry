...
Attribution properties are captured once per anonymous user by the Swift SDK and carried through the claim merge into the real user. To opt out, set `attributionEnabled: false` on `Owl.configure()`. See [Attribution](/docs/concepts/attribution) for the full pipeline.
