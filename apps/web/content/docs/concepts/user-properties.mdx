...
| `asa_campaign_name`, `asa_ad_group_name`, `asa_keyword` | [RevenueCat integration](/docs/concepts/integrations) backfill | Human-readable names for the same campaign / ad group the `asa_*_id` fields identify, plus the literal search term. Apple's AdServices Attribution API returns only numeric IDs — these name fields are populated by RevenueCat, which resolves Apple's IDs to names via Apple's ASA Campaign Management API (requires RevenueCat's "Advanced" Apple AdServices integration to be configured with ASA API credentials). |
...
The RC backfill path is additive: it never overwrites an `asa_*_id` or `attribution_source` set by the Swift SDK's live flow — it only fills empty slots. For users who predate the Apple AdServices capture, RC is the only source of attribution.
