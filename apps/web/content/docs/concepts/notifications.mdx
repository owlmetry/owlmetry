---
title: Notifications
description: Multi-channel notification system — per-user inbox, email, and iOS push fan-out from a single producer call.
---
...
| `ios_push` | APNs HTTP/2 to every registered iOS device. Unread count is computed and sent as the badge. Token-based auth with an Apple `.p8` key — see [APNs configuration](/docs/self-hosting/configuration#ios-push-apns) for env vars. |
