...
| Import | `owl_import_` | Bulk import scripts | Historical data import via `POST /v1/import` (project-scoped) |

### Security

API keys are stored securely and can be viewed at any time from the **API Keys** page in the dashboard.
...
| Bulk import script | `owl_import_` API key | `permissions` array on the key (project-scoped) |
