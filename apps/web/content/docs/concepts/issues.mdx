...
| `silenced` | Known issue, notifications suppressed. Occurrences still tracked. Use this when there's nothing to fix or no version exists. |
...
For this reason, `resolved_at_version` is **required** when transitioning an issue to `resolved` (across the dashboard, iOS app, CLI `--version`, MCP `version`, and the REST API). If you don't have a fix version — for example, the error was a transient infrastructure blip with nothing to fix — use `silenced` instead. Resolving without a version would silently disable regression detection for that issue, which is rarely what you want.
...
- **`issue.new`** — instant fan-out. The `issue_scan` job dispatches one notification per team at the end of every run summarizing all production issues created or regressed during that scan. Bypasses the per-project digest cadence so push lands in close to real time. Defaults to in-app + iOS push on, email off.
- **`issue.digest`** — per-project rollup. The `issue_notify` job assembles a periodic digest at each project's configured cadence and sends it to every team member over their enabled channels. Defaults to all three channels on.

Both types route through the unified [notifications dispatcher](/docs/concepts/notifications), so each recipient's per-channel toggles at `/dashboard/profile/notifications` decide whether a given delivery becomes an inbox row, an email, an iOS push, or some combination.
