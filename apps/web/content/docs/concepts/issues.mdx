...
- **Per-event-name discriminator** (optional): For some auto-emitted SDK events the fingerprint includes an extra discriminator so a single message name can produce multiple distinct issues. Today only `sdk:network_request` uses one — see below.
