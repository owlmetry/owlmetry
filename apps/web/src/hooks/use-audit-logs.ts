...
export function useAuditLogs(filters: AuditLogsQueryParams) {
  ...
  const key = filters.team_id ? `/v1/audit-logs${qs ? `?${qs}` : ""}` : null;
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get<AuditLogsResponse>(`/v1/audit-logs?${nextQs}`);
